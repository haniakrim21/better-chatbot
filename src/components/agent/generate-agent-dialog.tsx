...
export function GenerateAgentDialog({
  ...
  // Omit tools from the client validation to match the server-side generation schema
  // The server explicitly avoids generating tools to ensure stability (see api/agent/ai/route.ts)
  const schema = AgentGenerateSchema.omit({ tools: true });
  ...
  const { submit, isLoading, object } = experimental_useObject({
    ...
    onFinish(event) {
      ...
      if (event.object) {
        ...
        // Tools are not currently generated by the server for stability
        if (onToolsGenerated) {
          onToolsGenerated([]); // explicit empty list as fallback
        }
      }
